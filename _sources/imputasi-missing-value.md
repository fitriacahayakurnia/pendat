# Imputasi Missing Value dan Materi Normalisasi Data

## 1. Imputasi Missing Value Menggunakan Metode WKNN

Dalam pengolahan data, sering terdapat *missing value* yang dapat memengaruhi hasil analisis karena informasi menjadi tidak lengkap. Oleh karena itu, diperlukan metode untuk mengisi nilai yang hilang. Salah satu metode yang digunakan adalah **Weighted K-Nearest Neighbor Imputation (WKNN)**, yaitu pengembangan dari KNN yang mempertimbangkan kedekatan antar data serta bobot dari tetangga terdekat.

Pada metode ini, data yang memiliki nilai kosong dibandingkan dengan data lengkap, kemudian dihitung tingkat kemiripannya. Nilai yang hilang selanjutnya diperkirakan berdasarkan data yang paling mirip dengan menggunakan rata-rata berbobot.

#### Langkah-langkah metode WKNN

1. Mengidentifikasi data yang memiliki nilai hilang.
2. Menghitung tingkat kemiripan antara data tersebut dengan data lain.
3. Menentukan sejumlah tetangga terdekat (K).
4. Menentukan bobot berdasarkan nilai kemiripan.
5. Mengestimasi nilai yang hilang menggunakan rata-rata berbobot.
...
Langkah awal adalah mengidentifikasi baris data yang memiliki nilai yang hilang (missing value). Pada dataset Boston Housing, nilai yang hilang terdapat pada atribut **LSTAT**, yang akan diestimasi menggunakan metode WKNN.

#### 1.1.2  Menentukan Kolom yang Akan Dinormalisasi

Sebelum menghitung nilai kemiripan, data dinormalisasi terlebih dahulu agar seluruh atribut berada pada skala yang sama. Adapun kolom yang dinormalisasi meliputi:
...
Setelah proses normalisasi selesai, tahap berikutnya adalah menghitung nilai kemiripan antara data yang memiliki missing value dengan data lainnya.
...
Selanjutnya, nilai similarity dijumlahkan untuk memperoleh nilai penyebut dalam perhitungan imputasi.
...
Selanjutnya, dilakukan perkalian antara nilai similarity dengan nilai atribut dari data tetangga terdekat.

$$
S_i \times Y_{jh}
$$

Hasil perkalian tersebut kemudian dijumlahkan untuk memperoleh nilai pembilang.
...
Selain dihitung secara manual menggunakan Microsoft Excel, imputasi missing value juga dilakukan dengan Python untuk memverifikasi hasil serta mempermudah proses pengolahan data.
...
Nilai tersebut merupakan hasil imputasi untuk menggantikan missing value pada atribut LSTAT, sehingga dataset menjadi lengkap dan siap digunakan untuk analisis selanjutnya.

## 2. Normalisasi data

Normalisasi data merupakan proses penyesuaian skala nilai agar berada dalam rentang tertentu sehingga setiap variabel memiliki skala yang sebanding. Proses ini umumnya dilakukan pada tahap preprocessing sebelum analisis atau pemodelan.

Tujuan normalisasi adalah mencegah dominasi variabel dengan skala besar, sehingga setiap fitur dapat berkontribusi secara seimbang dalam perhitungan.

### 2.1 Macam-macam Normalisasi Data

#### 1. MinMax Normalization

Min-Max Normalization merupakan teknik normalisasi yang mengubah nilai data ke dalam rentang tertentu, umumnya antara 0 sampai 1.
...
**Contoh:**
Sebagai contoh digunakan sebagian data dari dataset Boston Housing pada kolom **TAX**.
...
1. Perhitungan normalisasi untuk data pertama:

$$
x' = \frac{296 - 222}{296 - 222}
$$
...
2. Perhitungan normalisasi untuk data kedua dan ketiga:

$$
x' = \frac{242 - 222}{296 - 222}
$$
...
3. Perhitungan normalisasi untuk data keempat:

$$
x' = \frac{222 - 222}{296 - 222}
$$
...
$$
(296 - 250.5)^2 = (45.5)^2 = 2070.25
$$

$$
(242 - 250.5)^2 = (-8.5)^2 = 72.25
$$

$$
(222 - 250.5)^2 = (-28.5)^2 = 812.25
$$

Jumlahkan:

$$
2070.25 + 72.25 + 72.25 + 812.25 = 3027
$$
...
- Mean ($\mu$) = 250.5
- Standar deviasi ($\sigma$) = 27.51

1. Perhitungan untuk data pertama:

$$
x' = \frac{296 - 250.5}{27.51}
$$
...
2. Perhitungan untuk data kedua dan ketiga:

$$
x' = \frac{242 - 250.5}{27.51}
$$
...
3. Perhitungan untuk data keempat:

$$
x' = \frac{222 - 250.5}{27.51}
$$
...
Menggunakan data yang sama pada kolom **TAX**.
...
Karena nilai terbesar memiliki **3 digit (sebelum desimal)**, maka:
...
Pada contoh berikut digunakan sebagian data dari **Boston Housing Dataset** pada kolom **TAX**.
...
data = {'TAX': [296, 242, 242, 222]}
df = pd.DataFrame(data)

scaler = MinMaxScaler()

df['TAX_MinMax'] = scaler.fit_transform(df[['TAX']])
...
df['TAX_ZScore'] = scaler.fit_transform(df[['TAX']])
...
max_value = df['TAX'].max()

j = len(str(int(max_value)))

df['TAX_Decimal'] = df['TAX'] / (10**j)
